...
namespace Classes
{
  public static class ACMClass
  {
    ...
    public static bool RegisterUser(
     int UserTenent, int Userid, int Locationid,
     string FIRST_NAME, string LAST_NAME, string FIRST_NAME1, string LAST_NAME1,
     string FIRST_NAME2, string LAST_NAME2, string LOGIN_ID, string PASSWORD,
     string PASSWORD_CHNG, int USER_TYPE, string REMARKS, string ACTIVE_FLAG,
     DateTime LAST_LOGIN_DT, string ACC_LOCK, string FIRST_TIME, string EmailAddress,
     string Avtar, DateTime Till_DT, int CompId, int USER_DETAIL_ID,
     bool ACTIVEUSER, DateTime USERDATE, string Flage, string Lang,
     // USER_DTL
     int COUNTRY, string STATE, string PH_NO, string FAX_NO, string PINCODE_NO,
     string POST_OFFICE, long MOBILE_NUM, string SEC_QUES, string SEC_ANS,
     // Privilege Copy
     int FromUserID, DateTime ActiveTillDate
 )
    {
      ...
      using (SqlCommand cmd = new SqlCommand("Register_User", conn))
      {
        ...
        cmd.Parameters.Add("@Lang1", SqlDbType.NVarChar, 10).Value = Lang ?? (object)DBNull.Value;
        ...
        cmd.Parameters.Add("@ActiveTillDate", SqlDbType.Date).Value = ActiveTillDate;
    ...
    public static void InsertDataACMMODULEMAP(int TenentID, int LOCATION_ID, int PRIVILEGE_ID, int MODULE_ID, int UserID, int MODULE_MAP_ID = 0, string ACTIVE_FLAG = "", int CRUP_ID = 1, int MySerial = 999999999, string ALL_FLAG = "", string ADD_FLAG = "", string MODIFY_FLAG = "", string DELETE_FLAG = "", string VIEW_FLAG = "")
    {
      Database.CallEntities DB = new Database.CallEntities();
      bool flag = false;
    ...
    public static void InsertWeb_User_MST(
    int UserTenent, int Userid, int Locationid,
    string FIRST_NAME, string LAST_NAME, string FIRST_NAME1, string LAST_NAME1,
    string FIRST_NAME2, string LAST_NAME2, string LOGIN_ID, string PASSWORD,
    string PASSWORD_CHNG, int USER_TYPE, string REMARKS, string ACTIVE_FLAG,
    ...
    bool ACTIVEUSER, DateTime USERDATE, string Flage, string Lang)
    {
      using (var DB = new Database.CallEntities())
      {
        Database.USER_MST objUSER_MST;

        if (Flage == "ADD")
        {
          objUSER_MST = new Database.USER_MST();
          DB.USER_MST.AddObject(objUSER_MST); // use Add() if EF6
        }
        else
        {
          objUSER_MST = DB.USER_MST
                         .SingleOrDefault(p => p.USER_ID == Userid && p.TenentID == UserTenent);
        ...
        objUSER_MST.USER_ID = Userid; // remove if it's identity column
